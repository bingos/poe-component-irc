...
=head1 DESCRIPTION
 ...
 use constant {
     ...
     USER_ACTIVITY => 1,
     DATA_FILE     => 'seen',
     SAVE_INTERVAL => 60 * 20,   # save state every 20 mins
 };
 ...
 sub irc_part {
     ...
     my $activity = 'parting $chan';
     $activity .= " with message '$text'" if defined $text;
     
     add_nick($nick, $activity);
 }
 ...
 sub irc_botcmd_seen {
     ...
     if ($seen->{l_irc($target)}) {
         ...
         my $activity = $seen->{l_irc($target)}->[USER_ACTIVITY];
         $irc->yield(privmsg => $channel, "$nick: I last saw $target at $date $activity");
     }
     else {
